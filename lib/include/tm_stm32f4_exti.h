 ...
 * @link
http://stm32f4-discovery.com/2014/10/library-38-external-interrupts-for-stm32f4
 * @version v1.4
 * @ide     Keil uVision
 * @license GNU GPL v3
 ...
 *
 * You only need to specify GPIOx, pin and interrupt trigger you want to use and
 ...
 * Each GPIO_PIN_x from all GPIOx is connected to one line. In other words, PC0,
PA0, PB0, PJ0,...
 * are all connected to line 0, and so on. But you can use only one pin for one
line at a time.
 * So only PA0 at one time, or PD0 or PC0, but one a time.
 *
 * You can still use more lines at the same time. So let's say PA0 is line0 and
PC13 is line13.
 ...
 *
 * I write these functions in my library and use my own to handle specific line
for you for easly.
 * Sometimes there is need for use default handler names in your own, but 2
functions with same name
 * can not be used, so I made some settings.
 *
 * If you open defines.h file and add any of lines below, you will disable
handler line for interrupts.
 * By default, all handlers for lines are enabled. Disabled then with adding
lines below in defines.h file:
 *
@verbatim
...
@endverbatim
 *
 * If you need higher priority for external interrupts in NVIC, add lines below
in defines.h file and edit them
@verbatim
...
@verbatim
 ...
 Version 1.3
  ...
  - Now only one function handler TM_EXTI_Handler for all lines. No separate
handlers anymore.
 ...
 Version 1.1
  - March 08, 2015
  - Added function TM_EXTI_Handler, which is called anytime EXTI interrupt
occur,
       no matter on which GPIO_Pin. GPIO_Pin_x is passed as argument where x is
0 to 15
...
#define EXTI_NVIC_PRIORITY 0x0A
  ...
  */
   ...
   */
  typedef enum {
    TM_EXTI_Trigger_Rising = 0x00, /*!< Trigger interrupt on rising edge on
                                      line, pull down resistor active */
    TM_EXTI_Trigger_Falling, /*!< Trigger interrupt on falling edge on line,
                                pull up resistor active */
    TM_EXTI_Trigger_Rising_Falling /*!< Trigger interrupt on any edge on line,
                                      no pull resistor active */
  } TM_EXTI_Trigger_t;
   ...
   * at the same time.
   *         If you will try that, my function will return error to you, because
   * you have to detach GPIO_Line first and attach back on other GPIO port.
   *
   * @param  *GPIOx: GPIO port where you want EXTI interrupt line
   * @param  GPIO_Line: GPIO pin where you want EXTI interrupt line. Use OR (|)
   * operator if you want to attach
   *            interrupt in more than one GPIO pin at the same GPIOx at the
   * same time.
   * @param  trigger: Pin trigger source. This parameter can be a value of @ref
   * TM_EXTI_Trigger_t enumeration
   * @retval Attach result:
   *            - @arg TM_EXTI_Result_Ok: Everything ok, interrupt attached
   *            - @arg TM_EXTI_Result_Error: An error occurred, interrupt was
   * not attached
   * @note   If you use more than one GPIO_Pin with OR (|) operator at single
   * call and if GPIO_Pin can't be attached because
   *         there is already one GPIO_Pin at this line, function will return
   * error and other pins might not be initialized.
   *         If function return @arg TM_EXTI_Result_Ok, then all pins are
   * attached correctly.
   */
  TM_EXTI_Result_t TM_EXTI_Attach(GPIO_TypeDef * GPIOx, uint16_t GPIO_Line,
                                  TM_EXTI_Trigger_t trigger);

  /**
   * @brief  Detach GPIO pin from interrupt lines
   * @param  GPIO_Line: GPIO line you want to disable. Valid GPIO is GPIO_Pin_0
   * to GPIO_Pin_15.
   *         Use OR (|) operator if you want to detach interrupt in more than
   * one GPIO pin at the same GPIOx at the same time.
   ...
   *         It has no effect for internally connected peripherals (like RTC) to
   * EXTI line
   * @param  None
   * @retval None
   */
...
#define TM_EXTI_SoftwareInterrupt(GPIO_Line) (EXTI->SWIER |= (GPIO_Line))
   ...
   * @note   This function is called from TM EXTI library each time any
   * interrupt occurs on EXTI line.
   * @note   With __weak parameter to prevent link errors if not defined by
   * user.
   * @param  GPIO_Pin: GPIO Line where interrupt occurred so you can identify
   * what to do
   * @retval None
   */
