 ...
 * instead of Systick)
 * - MISC         			(Only if you want to use TIMx for delay instead
 * of Systick)
 * - TM TIMER PROPERTIES	(Only if you want to use TIMx for delay instead of
 * Systick)
 * - defines.h
 * - attributes.h
 */
 ...
 *
@verbatim
If you are using GCC compiler, then your microseconds delay is probably totally
inaccurate.
 ...
 * Timer also makes an interrupts every 1ms (for count time) instead of 1us as
it was before.
 * For "us" delay, timer's counter is used to count ticks. It makes a new tick
each "us".
 * Not all MCUs have all possible timers, so this lib has been designed that you
select your own.
 *
 ...
 *
 *
 * With this setting (using custom timer) you have better accuracy in "us"
delay.
 * Also, you have to know, that if you want to use timer for delay, you have to
 ...
 * As of version 2.4, delay library allows you to create custom timer which
count DOWN and when it reaches zero, callback is called.
 *
 * You can use variable settings for count, reload value and auto reload
feature.
 *
 * \par Changelog
 *
@verbatim
 ...
 Version 2.1
  - GCC compiler fixes
  - Still prefer that you use TIM for delay if you are working with ARM-GCC
compiler

 Version 2.0
  - November 28, 2014
  - Delay library has been totally rewritten. Because Systick is designed to be
used
...
@verbatim
 - STM32F4xx
 - STM32F4xx RCC:       Only if you want to use TIMx for delay instead of
Systick
 - STM32F4xx TIM:       Only if you want to use TIMx for delay instead of
Systick
 - MISC
 - defines.h
 - TM TIMER PROPERTIES: Only if you want to use TIMx for delay instead of
...
typedef struct
{
  ...
  uint32_t AutoReload; /*!< Set to 1 if timer should be auto reloaded when it
                          reaches zero */
  uint32_t CNT;        /*!< Counter value, counter counts down */
  uint8_t Enabled;     /*!< Set to 1 when timer is enabled */
  void (*Callback)(
    void*); /*!< Callback which will be called when timer reaches zero */
  void* UserParameters; /*!< Pointer to user parameters used for callback
                           function */
} TM_DELAY_Timer_t;
 ...
 * @note   This function will initialize Systick or user timer, according to
 * settings
 * @param  None
 * @retval None
 */
...
/**
 * @brief  Re-enables delay timer It has to be configured before with
 * TM_DELAY_Init()
 * @note   This function enables delay timer. It can be systick or user
 * selectable timer.
 * @param  None
 * @retval None
 */
 ...
 * @note   This function disables delay timer. It can be systick or user
 * selectable timer.
 * @param  None
 * @retval None
 */
 ...
 * @param  ReloadValue: Number of milliseconds when timer reaches zero and
 * callback function is called
 * @param  AutoReload: If set to 1, timer will start again when it reaches zero
 * and callback is called
 * @param  StartTimer: If set to 1, timer will start immediately
 * @param  *TM_DELAY_CustomTimerCallback: Pointer to callback function which
 * will be called when timer reaches zero
 * @param  *UserParameters: Pointer to void pointer to user parameters used as
 * first parameter in callback function
 * @retval Pointer to allocated timer structure
 */
TM_DELAY_Timer_t* TM_DELAY_TimerCreate(
  uint32_t ReloadValue, uint8_t AutoReload, uint8_t StartTimer,
  void (*TM_DELAY_CustomTimerCallback)(void*), void* UserParameters);
 ...
 * @note   Auto reload features is used for timer which starts again when zero
 * is reached if auto reload active
 * @param  *Timer: Pointer to @ref TM_DELAY_Timer_t structure
 ...
 * @param  *Timer: Pointer to @ref TM_DELAY_Timer_t structure
 * @param  AutoReloadValue: Value for timer to be set when zero is reached and
 * callback is called
 * @note   AutoReload feature must be enabled for timer in order to get this to
 * work properly
 * @retval Pointer to @ref TM_DELAY_Timer_t structure
 */
