 ...
 *
 * It allows you to easily enable interrupts for DMA streams, checking flags and
more.
 *
 * It is also designed in a ways that all interrupts that happens are handled by
this library. No worries for correct stream handler anymore!
 *
 * \par Customization
 *
 * Library implements ALL stream handlers (DMA1,2, streams 0 to 7, together is
this 14 stream handlers) but some of my libraries uses default stream handler,
like FATFS.
 * FATFS uses DMA2_Stream3 for interrupts for SDIO and in my FATFS library is
written function name DMA2_Stream3_IRQHandler().
 *
 * DMA library also features this function and now if you use DMA library and
FATFS library, you will get error for "Multiple declarations...".
 *
 * To prevent this link errors, you can open defines.h configuration file and
add defines like below:
 *
@verbatim
 ...
 *
 * Every stream on DMA can make 5 interrupts. My library is designed in a way
that specific callback is called for each interrupt type.
 ...
 */
#define DMA_FLAG_TCIF                                                          \
  ((uint32_t)0x00000020) /*!< DMA stream transfer complete */
#define DMA_FLAG_HTIF                                                          \
  ((uint32_t)0x00000010) /*!< DMA stream half transfer complete */
#define DMA_FLAG_TEIF ((uint32_t)0x00000008) /*!< DMA stream transfer error */
#define DMA_FLAG_DMEIF                                                         \
  ((uint32_t)0x00000004) /*!< DMA stream direct mode error */
 ...
 * @param  *DMA_Stream: Pointer to @ref DMA_Stream_TypeDef DMA stream where you
 * want to clear flags
 * @retval None
 */
