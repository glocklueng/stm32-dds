 ...
 * @link
http://stm32f4-discovery.com/2015/04/library-56-extend-spi-with-dma-for-stm32f4xx
 * @version v1.1.1
 * @ide     Keil uVision
 * @license GNU GPL v3
...
/**
 * @defgroup TM_SPI_DMA
 * @brief    DMA functionality for @ref TM_SPI library -
http://stm32f4-discovery.com/2015/04/library-56-extend-spi-with-dma-for-stm32f4xx
 * @{
 *
 * This library allows you to use DMA with SPI peripheral.
 *
 * It can send (TX only), receive (RX only) or transmit (TX and RX) data over
SPI with DMA feature.
 *
 * \par
 *
 * @note All possible DMA Streams and Channels for SPI DMA can be found in
STM32F4xx Reference manual.
 *
 * Default DMA streams and channels:
 *
@verbatim
SPIx     | DMA  | DMA TX Stream | DMA TX Channel | DMA RX Stream | DMA RX
Channel
...
/**
 * @brief  Initializes SPI DMA functionality with custom DMA stream and channel
 * options
 * @note   SPI HAVE TO be previously initialized using @ref TM_USART library
 *
 * @note   Use this function only in case default Stream and Channel settings
 * are not good for you
 * @param  *SPIx: Pointer to SPIx where you want to set custom DMA streams and
 * channels
 * @param  *TX_Stream: Pointer to DMAy_Streamx, where y is DMA (1 or 2) and x is
 * Stream (0 to 7)
 * @param  TX_Channel: Select DMA TX channel for your SPI in specific DMA Stream
 * @param  *RX_Stream: Pointer to DMAy_Streamx, where y is DMA (1 or 2) and x is
 * Stream (0 to 7)
 * @param  RX_Channel: Select DMA RX channel for your SPI in specific DMA Stream
 * @retval None
 */
void TM_SPI_DMA_InitWithStreamAndChannel(SPI_TypeDef* SPIx,
 ...
 * @note   Try not to use local variables pointers for DMA memory as TX and RX
 * Buffers
 ...
 * over SPI.
 *            Set this parameter to NULL, if you want to sent "0x00" and only
 * receive data into *RX_Buffer pointer
 * @param  *RX_Buffer: Pointer to RX_Buffer where DMA will save data from SPI.
 *            Set this parameter to NULL, if you don't want to receive any data,
 * only sent from TX_Buffer
 ...
 */
uint8_t TM_SPI_DMA_Transmit(SPI_TypeDef* SPIx, uint8_t* TX_Buffer,
                            uint8_t* RX_Buffer, uint16_t count);
 ...
 */
#define TM_SPI_DMA_Send(SPIx, TX_Buffer, count)                                \
  (TM_SPI_DMA_Transmit(SPIx, TX_Buffer, NULL, count))

/**
 * @brief  Sends dummy byte (0x00) over SPI to receive data back from slave over
 * DMA
 * @note   Try not to use local variables pointers for DMA memory as TX and RX
 * Buffers
 * @param  SPIx: Pointer to SPIx where DMA transmission will happen
 ...
 * @note   Defined as macro for faster execution
 */
#define TM_SPI_DMA_Receive(SPIx, RX_Buffer, count)                             \
 ...
 *            - > 0: DMA has started with sending data
 */
uint8_t TM_SPI_DMA_SendHalfWord(SPI_TypeDef* SPIx, uint16_t value,
                                uint16_t count);
 ...
 * @param  *SPIx: Pointer to SPIx peripheral where you want to enable DMA stream
 * interrupts
 * @retval None
 */
 ...
 * @param  *SPIx: Pointer to SPIx peripheral where you want to disable DMA
 * stream interrupts
 * @retval None
 */
