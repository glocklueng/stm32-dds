...
TM_SPI_GetPrescalerFromMaxFrequency()
 ...
 Version 1.5
  - January 13, 2015
  - Added function TM_SPI_InitWithMode() to initialize SPI with custom SPI mode
on the fly
...
typedef enum {
  ...
  TM_SPI_PinsPack_Custom /*!< Select custom pins for specific SPI, callback will
                            be called, look @ref TM_SPI_InitCustomPinsCallback
                            */
} TM_SPI_PinsPack_t;
...
/**
 * @brief  Initializes SPIx peripheral with custom pinspack and default other
 * settings
 * @param  *SPIx: Pointer to SPIx peripheral you will use, where x is between 1
 * to 6
 * @param  pinspack: Pinspack you will use from default SPI table. This
 * parameter can be a value of @ref TM_SPI_PinsPack_t enumeration
 * @retval None
 */
void TM_SPI_Init(SPI_TypeDef* SPIx, TM_SPI_PinsPack_t pinspack);

/**
 * @brief  Initializes SPIx peripheral with custom pinspack and SPI mode and
 * default other settings
 * @param  *SPIx: Pointer to SPIx peripheral you will use, where x is between 1
 * to 6
 * @param  pinspack: Pinspack you will use from default SPI table. This
 * parameter can be a value of @ref TM_SPI_PinsPack_t enumeration
 * @param  SPI_Mode: SPI mode you will use. This parameter can be a value of
 * @ref TM_SPI_Mode_t enumeration
 * @retval None
 */
void TM_SPI_InitWithMode(SPI_TypeDef* SPIx, TM_SPI_PinsPack_t pinspack,
 ...
 * @param  pinspack: Pinspack you will use from default SPI table. This
 * parameter can be a value of @ref TM_SPI_PinsPack_t enumeration
 * @param  SPI_BaudRatePrescaler: SPI baudrate prescaler. This parameter can be
 * a value of @ref SPI_BaudRatePrescaler
 * @param  SPI_Mode_t: SPI mode you will use. This parameter can be a value of
 * @ref TM_SPI_Mode_t enumeration
 ...
 * @retval None
 */
void TM_SPI_InitFull(SPI_TypeDef* SPIx, TM_SPI_PinsPack_t pinspack,
                     uint16_t SPI_BaudRatePrescaler, TM_SPI_Mode_t SPI_Mode_t,
                     uint16_t SPI_Mode, uint16_t SPI_FirstBit);

/**
 * @brief  Calculates bits for SPI prescaler register to get minimal prescaler
 * value for SPI peripheral
 * @note   SPI has 8 prescalers available, 2,4,6,...,128,256
 * @note   This function will return you a bits you must set in your CR1
 * register.
 *
 * @note   Imagine, you can use 20MHz max clock in your system, your system is
 * running on 168MHz, and you use SPI on APB2 bus.
 *         On 168 and 180MHz devices, APB2 works on Fclk/2, so 84 and 90MHz.
 *         So, if you calculate this, prescaler will need to be 84MHz / 20MHz =
 * 4.xx, but if you use 4 prescaler, then you will be over 20MHz.
 *         You need 8 prescaler then. This function will calculate this.
 * @param  *SPIx: Pointer to SPIx peripheral you will use, where x is between 1
 * to 6.
 *            Different SPIx works on different clock and is important to know
 * for which SPI you need prescaler.
 * @param  MAX_SPI_Frequency: Max SPI frequency you can use. Function will
 * calculate the minimum prescaler you need for that.
 *
 * @retval Bits combination for SPI_CR1 register, with aligned location already,
 * prepared to set parameter for @ref TM_SPI_InitFull() function.
 */
 ...
 * @param  DataSize: Datasize which will be used. This parameter can be a value
 * of @ref TM_SPI_DataSize_t enumeration
...
TM_SPI_Send16(SPI_TypeDef* SPIx, uint8_t data)
{
  /* Check if SPI is enabled */
  SPI_CHECK_ENABLED_RESP(SPIx, 0);
 ...
 * @retval None
 */
void TM_SPI_SendMulti16(SPI_TypeDef* SPIx, uint16_t* dataOut, uint16_t* dataIn,
                        uint32_t count);
 ...
 * @param  dummy: Dummy 16-bit value to be sent over SPI, to receive data back.
 * In most cases 0x00 or 0xFF
 * @param  count: Number of 16-bit values you want read from device
 * @retval None
 */
void TM_SPI_ReadMulti16(SPI_TypeDef* SPIx, uint16_t* dataIn, uint16_t dummy,
                        uint32_t count);

/**
 * @brief  Init custom SPI pins for your SPIx. This is callback function and
 * will be called from my library if needed.
 * @note   When you call TM_SPI_Init() function, and if you pass
 * TM_SPI_PinsPack_Custom to function,
 *         then this function will be called where you can initialize custom
 * pins for SPI peripheral
 *
 * @note   You have to initialize MOSI, MISO and SCK pin
 *
 * @param  *SPIx: Pointer to SPIx peripheral for which you have to set your
 * custom pin settings
