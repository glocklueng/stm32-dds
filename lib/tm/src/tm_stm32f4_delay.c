...
SysTick_Handler(void)
{
  ...
  for (i = 0; i < CustomTimers.Count; i++) {
    ...
    if (CustomTimers.Timers[i] &&          /*!< Pointer exists */
      ...
      if (CustomTimers.Timers[i]->CNT == 0) {
        ...
        CustomTimers.Timers[i]->Callback(
          CustomTimers.Timers[i]->UserParameters);
...
TM_DELAY_INT_InitTIM(void)
{
  ...
  TIM_TimeBaseStruct.TIM_Prescaler =
    SystemCoreClock / (1000000 * (SystemCoreClock / TIM_Data.TimerFrequency)) -
    1; /* With prescaler for 1 microsecond tick */
...
TM_DELAY_Timer_t*
TM_DELAY_TimerCreate(uint32_t ReloadValue, uint8_t AutoReload,
                     uint8_t StartTimer,
