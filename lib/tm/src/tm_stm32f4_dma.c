...
#define GET_STREAM_NUMBER_DMA1(stream)                                         \
  (((uint32_t)(stream) - (uint32_t)DMA1_Stream0) / (0x18))
#define GET_STREAM_NUMBER_DMA2(stream)                                         \
  (((uint32_t)(stream) - (uint32_t)DMA2_Stream0) / (0x18))

/* Offsets for bits */
const static uint8_t DMA_Flags_Bit_Pos[4] = { 0, 6, 16, 22 };
...
TM_DMA_ClearFlag(DMA_Stream_TypeDef* DMA_Stream, uint32_t flag)
{
  ...
  /* Clear flags */
  *(__IO uint32_t*)location = (flag & DMA_FLAG_ALL)
...
{
  /* NOTE: This function should not be modified, when the callback is needed,
      the TM_DMA_HalfTransferCompleteHandler could be implemented in the user
     file
  */
}
