...
TM_EXTI_Result_t
TM_EXTI_Attach(GPIO_TypeDef* GPIOx, uint16_t GPIO_Line,
               TM_EXTI_Trigger_t trigger)
{
  ...
  if ((EXTI->IMR & GPIO_Line) || /*!< Interrupt already attached */
  ...
  /* Init GPIO pin */
  TM_GPIO_Init(GPIOx, GPIO_Line, TM_GPIO_Mode_IN, TM_GPIO_OType_PP, PuPd,
               TM_GPIO_Speed_Low);
